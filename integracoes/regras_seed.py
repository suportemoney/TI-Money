...
REGRAS_SEED: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        'conteudo': (
            ...
            '- WhatsApp/chip: consulte consultar_chips pelo nome E, se o solicitante passar '
            'um número, consulte de novo com esse número (aceita com DDI/espaços). '
            'A busca por nome usa o titular atual (entrega ou transferência). '
            'Liste todos os em uso na pergunta — não omita. '
            'Se o número informado não estiver na lista ou o titular (employee_name) for '
            'diferente do nome do chamado: NÃO escalar na hora; use enviar_pergunta_opcoes '
            'ou enviar_esclarecimento (ex.: confirmar se o chip foi transferido, se o nome '
            'no sistema está errado, ou se é outro consultor). '
            'Só escalar_para_ti depois de validar ou se a TI precisar agir (código WhatsApp). '
            'Status/obs: atualizar_status_chip / atualizar_observacao_chip; '
            'se já tiver 2 em uso, questione qual pede código. '
            ...
            'Nunca recuse por falta de resposta.\n'
            ...
            '- Só use RESOLVED se o solicitante/criador informar que já foi resolvido '
            'ou pedir para fechar, ou se um membro TI (eh_membro_ti) pedir. '
            'Nunca finalize por silêncio/falta de resposta.\n'
